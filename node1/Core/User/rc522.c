/****************************************功能说明****************************************
1、CubeMX 定义任意两个引脚，作为复位脚(RST)和片选脚(SDA),并对引脚作出如下配置：
   
	 只需要配置为输出模式即可,其他默认
   ---------------------------------------------------------
   开启SPI功能，模式选择-->Full-Duplex Master(全双工)，其他配置如下：
   Prescaler(for Baud Rate)－－＞32,其他默认

2、接线方式：
SPI_MISO（MUC）－－＞ MISO(器件),不需要反接
   SPI_MOSI（MUC）－－＞ MOSI(器件),不需要反接
   其他引脚一一对应
...
#define RS522_RST(N) HAL_GPIO_WritePin(GPIO_OUT2_GPIO_Port, GPIO_OUT2_Pin, N==1?GPIO_PIN_SET:GPIO_PIN_RESET)
#define RS522_NSS(N) HAL_GPIO_WritePin(GPIO_OUT3_GPIO_Port, GPIO_OUT3_Pin, N==1?GPIO_PIN_SET:GPIO_PIN_RESET)
