...
uint8_t co2_receive_msg(uint16_t *rec_msg)
{
  uint8_t ret_val= FALSE;
	...
	check_value = uart3_buf[0]+uart3_buf[1]+uart3_buf[2]+uart3_buf[3]+uart3_buf[4];
		
	if(check_value==uart3_buf[5])
	{
		*rec_msg=((uart3_buf[1]<<8)|uart3_buf[2]);
		//printf("co23:%d\r\n",*rec_msg);
		ret_val= TRUE;
	} else {
    ret_val= FALSE;
